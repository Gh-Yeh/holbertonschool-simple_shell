...
/**
 * func_dup_chars - duplicates characters
 ...
 */
char *func_dup_chars(char *pathstr, int start, int stop)
{
 ...
 */
char *func_find_path(info_t *info, char *pathstr, char *cmd)
{
	int i = 0, curr_pos = 0;
	char *path;
	...
	while (1)
	{
		...
		{
			path = func_dup_chars(pathstr, curr_pos, i);
			if (!*path)
				_strcat(path, cmd);
			else
			{
